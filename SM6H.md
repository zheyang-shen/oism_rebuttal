...
$${\color{red}\text{MR. Check this please.}}$$ _MR_ Just to clarify what you mean with the sentence above. The figure in the paper was produced via non-truncated BM and now you suggest using truncated BM to compute a measure of 'ground-truth' (upon truncation of the series)? Or the one in the paper was already via truncated BM and we need to include more terms in the series to make it closer to ground-truth? In general, I am keen on removing this sentence, as it might cause confusion, and it seems not needed for addressing the question of theoretical study. 

_ZS_: The ground truth score function of that Gaussian mixture distribution has infinite support, but the forward process dictates that it has finite support and we are essentially learning a density function defined on the interval, and the "tail" density gets folded into the interval which changes the score function. I think the inaccuracy of Figure 1 needs to be addressed, otherwise OISM I seems like it fits the ground truth score poorly, even when every other condition is ideal. 
